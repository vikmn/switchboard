...
# By default it makes live checks (aws sts, gcloud token, gh api) for the
# ●/○ status. Pass --fast (or -f) to skip all network calls and show just the
# configured names/profiles instantly (status shown as '?').
function env-status {
  local fast=""
  [[ "$1" == "--fast" || "$1" == "-f" ]] && fast=yes
